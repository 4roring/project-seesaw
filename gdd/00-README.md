...
| [02-boss-intent-thresholds.md](02-boss-intent-thresholds.md) | 파훼 임계점 — 적마다 다른 `적max`와 임계점 조작 |
...
| [10-enemy-roster.md](10-enemy-roster.md) | 적 10종 — 개성을 만드는 두 축(`적max`·클로저 성격)과 스테이지 편성 |
...
- **핵심 축:** `아6 ~ 0 ~ 적max` 단일 기세. 아 쪽은 6칸 고정, 적 쪽은
  **적마다 폭이 다릅니다**(4~7) — 그게 곧 파훼 임계점입니다
...
- **목표:** 연계를 쌓아 기세를 `적max`까지 밀어 파훼하고, 피해를 누적해
  상대를 쓰러뜨리면 승리
- **적 10종의 개성:** `적max`(파훼가 현실적인가)와 클로저 성격(버퍼 축적이
