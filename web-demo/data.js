...
const TS_DATA = {
  ...
  //
  // 병기는 한 자루만 쥔다. 새 병기를 쥐려면 쥔 것을 놓는다 (gdd/14 14-2).
  // ─────────────────────────────────────────────────────────────
  WEAPONS_ENABLED: true,
  WEAPONS: {
    sword: {
      key: 'sword', name: '검(劍)', icon: '🗡',
    ...
    gauntlet: {
      key: 'gauntlet', name: '권갑(拳甲)', icon: '👊',
      flavor: '막지 않는 주먹이 가장 먼저 닿는다.',
      rule: '방어도 없이 합을 넘기면 다음 합 첫 공격 초식 피해 +3',
      // 자원: 남은 방어도 — 곤(棍)을 반대로 읽는다. 곤은 방어도를 "남긴 채"
      // 넘기면 다음 합이 깊어지고, 이쪽은 "하나도 안 남기고" 넘기면 다음 합의
      // 첫 공격이 무거워진다. 병기는 한 자루라 둘이 겹쳐 쌓일 일은 없다.
      // 맨손의 자리를 대신한다 (gdd/14 14-2). 테마는 금강사의 권·장이지만
      // 금강사는 방어도를 쌓는 색이라 수치로는 가장 안 맞을 수 있다 (14-4).
      // +8은 초심자 4색 × 90런 평균 5.40, 마교 6.7로 혼자 튀었다. 마교·무당은
      // 거의 막지 않아 매 합 터지고(합당 0.6~0.7회), 금강사는 방어도를 남겨
...
// ── 규칙 설명 문구 ────────────────────────────────────────────
// 전투 화면의 "규칙" 창과 guide.html이 같은 배열을 읽는다. 같은 문장을 두
// 곳에 쓰면 수치를 조정할 때 한쪽만 고쳐 놓고 못 알아챈다 — 이번에 강화표
// 에서 똑같은 종류의 조용한 오류를 셋 찾았다. 숫자는 전부 위 상수에서
// 끌어오므로, 상수를 바꾸면 설명도 같이 바뀐다.
//
// 객체 리터럴 안이 아니라 밖에 두는 이유: 리터럴을 정의하는 중에는
// TS_DATA.DRAW_ACTION_COST를 읽을 수 없다.
TS_DATA.RULES = [
  {
    title: '기세는 하나를 둘이 나눠 씁니다',
    lines: [
      `축은 <b>아${-TS_DATA.GAUGE_MIN} … 0 … 적${TS_DATA.GAUGE_MAX}</b> 하나뿐입니다. 내 자원이 아니라 <b>시소</b>입니다.`,
      '초식을 쓰면 그 초식의 <b>틈</b>만큼 기세가 적 쪽으로 밀립니다.',
      // "밀어낸 만큼 되돌려 준다"고 먼저 썼다가 실측에서 뒤집혔다. 깊이는
      // 되돌려 받는 양이 아니라 적이 쓸 예산이다 (적1로 넘겨도 아4로
      // 돌아오고, 적6으로 넘겨도 아3이다).
      '<b>0을 넘기는 순간</b> 선(先)이 적에게 넘어갑니다. 이때 <b>내가 밀어낸 깊이가 그대로 적의 예산</b>이 되어, 깊게 넘긴 만큼 적이 많이 움직입니다.',
      '깊게 밀면 그 합에 많이 하지만 크게 맞습니다. <b>0에 딱 맞추면 선을 넘기지 않아 내 합이 이어집니다.</b>',
      `돌아오는 기세는 아${TS_DATA.MIN_MOMENTUM_RETURN} 안팎으로 거의 일정합니다 — <b>깊게 넘겨서 얻는 것은 없습니다.</b> 기본은 얕게.`,
    ],
  },
  {
    title: '파훼는 한 합의 합계로 터집니다',
    lines: [
      '이번 합에 초식으로 지불한 틈을 <b>모두 더한 값</b>이 몰아치기입니다. 이것이 적의 <b>빈틈</b>에 닿으면 <b>파훼</b>.',
      '파훼하면 적의 반격이 통째로 취소되고, 다음 합에는 사혈이 드러나 적이 받는 피해가 늘어납니다.',
      '조금씩 나눠 때리면 아무 일도 일어나지 않습니다 — <b>한 합에 몰아쳐야</b> 합니다.',
      '<b>숨 고르기는 몰아치기에 들어가지 않습니다.</b> 물러서며 숨을 고르는 행동으로 상대 초식을 깨뜨릴 수는 없습니다.',
    ],
  },
  {
    title: '그 밖에',
    lines: [
      `<b>숨 고르기</b> — 기세 ${TS_DATA.DRAW_ACTION_COST}를 내주고 카드 ${TS_DATA.DRAW_ACTION_CARDS}장. 손패가 말라도 할 일이 없는 합은 없습니다.`,
      `<b>연환</b> — 한 합에 초식 ${TS_DATA.COMBO_THRESHOLD}장을 이으면 카드 1장을 더 뽑습니다.`,
      `적 페이즈가 끝나면 기세는 최소 <b>아${TS_DATA.MIN_MOMENTUM_RETURN}</b>으로 돌아옵니다 — 아무리 얕게 넘겨도 다음 합은 있습니다.`,
    ],
  },
];

// ── 용어 사전 ─────────────────────────────────────────────────
// guide.html의 용어표와 전투 화면의 용어 툴팁이 같은 배열을 읽는다.
// 넣는 기준: 화면에 실제로 나오고, 한국어를 알아도 뜻을 짐작할 수 없는 것.
// 방어도·피해·회복처럼 장르 표준인 말은 넣지 않는다 (gdd/12 12-3).
TS_DATA.GLOSSARY = [
  { term: '기세', desc: `둘이 나눠 쓰는 하나의 축(아${-TS_DATA.GAUGE_MIN} … 0 … 적${TS_DATA.GAUGE_MAX}). 0을 넘기면 선이 넘어간다.` },
  { term: '틈', desc: '초식을 쓰는 값. 그만큼 기세가 적 쪽으로 밀린다.' },
  { term: '초식', desc: '카드 한 장. 무공의 한 동작.' },
  { term: '합', desc: '주고받는 한 차례. 내가 선을 넘기면 적의 페이즈가 돌아간다.' },
  { term: '선(先)', desc: '지금 행동할 권리. 기세가 0을 넘어가는 순간 넘어간다.' },
  { term: '빈틈', desc: '적을 무너뜨리려면 한 합에 몰아쳐야 하는 틈의 양. 적마다 다르다.' },
  { term: '몰아치기', desc: '이번 합에 초식으로 지불한 틈의 합계. 숨 고르기는 세지 않는다.' },
  { term: '파훼', desc: '몰아치기가 빈틈에 닿는 것. 적의 반격이 통째로 취소된다.' },
  { term: '숨 고르기', desc: `기세 ${TS_DATA.DRAW_ACTION_COST}를 내주고 카드 ${TS_DATA.DRAW_ACTION_CARDS}장. 손패가 말랐을 때의 퇴로.` },
  { term: '연환', desc: `한 합에 초식 ${TS_DATA.COMBO_THRESHOLD}장을 이으면 카드 1장 추가.` },
  { term: '무너짐', desc: '한 합간 아무 초식도 쓰지 못하는 상태. 파훼가 만든다.' },
  { term: '사혈 노출', desc: '받는 피해가 늘어난 상태. 파훼 다음 합의 적이 이렇게 된다.' },
  { term: '내상', desc: '주는 피해가 줄어든 상태.' },
  { term: '되감기', desc: '기세를 내 쪽으로 되돌린다. 한 합을 늘려 더 몰아칠 수 있게 한다.' },
  { term: '반탄', desc: '적 페이즈 동안 맞을 때마다 되돌려 주는 피해.' },
  { term: '흘리기', desc: '다음 피격을 통째로 넘겨보낸다. 깎는 게 아니라 무효로 만든다.' },
  { term: '심법', desc: '몇 합간 이어지는 자기 강화.' },
  { term: '경지', desc: '초식의 등급. 삼류에서 화경까지, 이름의 규모로 읽힌다.' },
  { term: '신병이기', desc: '한 합 안의 규칙을 비트는 무기. 문파와 곱해져 두 번째 축이 된다.' },
  { term: '기물', desc: '강호행 전체의 규칙을 비트는 물건. 무덤과 기연에서 얻는다.' },
